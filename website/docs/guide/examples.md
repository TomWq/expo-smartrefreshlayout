---
title: 完整示例
description: Promise 自动结束、Lottie 自定义 Header、受控状态、实例命令、自动加载和 Android 淘宝二楼的完整示例。
---
...
仓库中的 [Classic 页面](https://github.com/TomWq/expo-smartrefreshlayout/blob/main/example/app/classic.tsx)
和 [Material 页面](https://github.com/TomWq/expo-smartrefreshlayout/blob/main/example/app/material.tsx)
是 Expo Router 路由，可直接切换 Spinner、波浪、内容偏移和主题色。
...
完整的 Expo Router 示例位于
[example/app/lottie.tsx](https://github.com/TomWq/expo-smartrefreshlayout/blob/main/example/app/lottie.tsx)。
下面的独立组件展示整个生命周期：`pulling`/`ready` 时 `progress` 跟随原生下拉和回弹；刷新时去掉
受控 `progress` 并使用 `play()` + `loop`；完成后 `pause()` + `reset()`。不能只在
`isDragging=true` 时更新进度，否则取消下拉时不会回到首帧。
